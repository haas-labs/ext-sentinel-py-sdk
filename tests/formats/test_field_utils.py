...
def test_transaction_data_merge():
    ...
    transaction = dicts_merge(block_data, transaction_data)
    transaction = dicts_merge(transaction, transaction_receipt_data)
...
def test_transaction_field_filter():
    ...
    assert sorted(dict_fields_filter(transaction, ignore_list=JSONRPC_TRANSACTION_FIELD_IGNORE_LIST).keys()) == sorted(
        [
            "blockHash",
            "blockNumber",
            "contractAddress",
...
def test_transaction_field_mappings():
    ...
    source_transaction = dicts_merge(block_data, transaction_data)
    source_transaction = dicts_merge(source_transaction, transaction_receipt_data)

    filtered_source_transaction = dict_fields_filter(source_transaction, JSONRPC_TRANSACTION_FIELD_IGNORE_LIST)

    target_transaction = dict_fields_mapping(filtered_source_transaction, JSONRPC_TRANSACTION_FIELD_MAPPINGS)
