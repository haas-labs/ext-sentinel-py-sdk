...
class ServiceAccountToken:
    ...
    def fetch(self) -> Token:
        ...
        response = httpx.post(url=endpoint,
                              data=params,
                              headers=headers,
                              verify=False)
...
def import_service_tokens():
    ...
    for token_name, vars in SERVICE_ACCOUNT_TOKENS.items():
        realm = os.environ.get(vars.get('realm')) \
                        if vars.get('realm') is not None else None
        client_id = os.environ.get(vars.get('client_id')) \
                        if vars.get('client_id') is not None else None
        client_secret = os.environ.get(vars.get('client_secret')) \
                        if vars.get('client_secret') is not None else None
        sa_token = ServiceAccountToken(endpoint_url=endpoint,
                                       realm=realm,
                                       client_id=client_id,
